...
function incomeAt(
  ...
  phase: TidePhase,
  tideStep: number,
  /** The turn number this income is being priced for. */
  ...
  // Compensation for moving second, paid on that player's first turn.
  //
  // Computed here rather than added by the resolver afterwards, which is where
  // it used to live — and because the receipt shown to the player is built from
  // these lines, an amount paid outside them was silently missing from
  // "earned this turn". Anything that reaches the bank has to be a line.
  if (playerId !== state.startingPlayer && turn <= 2) {
    const bonus = state.config.secondPlayerBonusEnergy;
    if (bonus > 0) {
      lines.push({
        source: 'compensation',
        amount: bonus,
        detail: 'for taking the second turn',
      });
    }
  }
...
export function energyIncome(state: GameState, playerId: PlayerId): EnergyIncome {
  return incomeAt(state, playerId, state.phase, state.tideStep, state.turn);
}
...
export function nextTurnIncome(state: GameState, playerId: PlayerId): EnergyIncome {
  ...
  // Their next turn is one or two turns from here, depending on whose it is now.
  const turn = state.turn + (state.activePlayer === playerId ? 2 : 1);
  return incomeAt(state, playerId, phaseAtTurnOf(state, playerId), state.tideStep + steps, turn);
}
